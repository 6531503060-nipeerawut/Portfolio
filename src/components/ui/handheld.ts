/**
 * The phone app's class vocabulary.
 *
 * This is the handheld half of the site and it shares nothing visual with
 * `components/ui/document.ts`. That file describes a document — glass panels,
 * hover lifts, magnetic buttons, a cursor spotlight, sections a screen tall
 * that a mouse wheel travels through. None of that means anything to a thumb,
 * and half of it costs a phone real frames.
 *
 * What replaces it is the vocabulary a native app already uses: solid surfaces
 * instead of blurred ones, list rows instead of hover cards, pressed states
 * instead of hovered ones, and full-width controls sized for a finger rather
 * than a pointer.
 *
 * Where the app's screens *are* — APP_ROOT, appHref, TABS — is a routing fact,
 * not a visual one, and lives in `constants/navigation.ts`.
 *
 * These are Tailwind classes, not a stylesheet. Tailwind scans this file like
 * any other source file, so every class below has to be written out in full —
 * nothing here may be assembled from fragments at runtime.
 */
