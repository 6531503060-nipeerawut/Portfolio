...
const FIGURE_LABEL =
  'mt-[.35rem] block font-mono text-[.6rem] tracking-[.1em] uppercase text-ink-faint';
...
/**
 * The app's `/m/user` screen.
 *
 * Same account as the document reads, same call — see AccountScreen for why
 * the two renderings live in one feature. What differs is everything visual:
 * a centred card instead of a wide one, three tiles instead of a row of
 * figures, and a full-width button instead of a link with a hover.
 */
export async function AccountAppScreen() {
  ...
  return (
    <div className={`${SCREEN} ${STACK}`}>
      <header className={`${APP_ENTER} pt-6 pb-1`}>
        ...
        <p className={APP_LEAD}>
          Read live from GitHub, so the counts here are whatever they are today.
        </p>
      </header>
      ...
      {user ? (
        <>
          <section
            className={`${APP_PANEL} ${APP_ENTER} text-center`}
          ...
          <section
            className={`${APP_ENTER} grid grid-cols-3 gap-[.6rem]`}
