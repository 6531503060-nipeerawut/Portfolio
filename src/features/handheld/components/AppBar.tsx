...
import { APP_ICON_BTN } from '@/components/ui/handheld';
import { ACCOUNT_APP_HREF, APP_ROOT, TABS } from '@/constants/navigation';
...
/**
 * The palette, as an external store.
 *
 * `data-theme` on `<html>` is the real state: the head script writes it before
 * React exists, the stylesheet reads it, and this bar only reports it. Holding
 * a copy in `useState` would mean two answers to one question, and the copy
 * would be wrong whenever something other than this button changed the
 * attribute — the bootstrap on a back-forward restore, or a second tab.
 *
 * Subscribing to the attribute instead keeps one answer, and removes the
 * mount-time `setState` that made the bar render twice on every navigation.
 */
...
/**
 * Null on the server. The palette is resolved by the head script from
 * localStorage, which the server cannot read, so rendering a label for it
 * server-side would be a guess — and a guess React would then report as a
 * hydration mismatch. The two glyphs need no state at all: the stylesheet
 * picks between them off `data-theme`, so the button is correct on the first
 * paint and only its label waits.
 */
...
export function AppBar() {
  ...
  function toggle() {
    ...
    // No setState: painting changes the attribute, the observer above sees it,
    // and the label follows from the one place the palette is actually kept.
    paint(next);
  }
