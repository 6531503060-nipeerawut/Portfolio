...
import { HEADINGS, TIMELINE } from '@/constants/content';
import {
  CARD,
  GRAD,
  H2,
...
const TL_HEAD =
  'mb-[.7rem] short:mb-2 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-[.4rem] max-[640px]:flex-col max-[640px]:items-start';
...
const TL_POINT =
  'flex gap-[.6rem] text-[.89rem] short:text-[.86rem] text-ink-muted [&_svg]:mt-[.28rem] [&_svg]:size-4 [&_svg]:flex-none [&_svg]:text-brand-3';
              ...
              <article className={TL_CARD}>
                <header className={TL_HEAD}>
                  ...
                  <div className="flex flex-wrap items-baseline gap-[.4rem]">
                    {role.when.map((when) => (
                      <span className={TL_WHEN} key={when}>
                        {when}
                      </span>
                    ))}
                  </div>
