...
import { MARQUEE, PROFILE, SOCIALS } from '@/constants/content';
import { BTN_GHOST, BTN_PRIMARY, ENTER, EYEBROW, GRAD, SHELL } from '@/components/ui/document';
import { PROFILE_IMAGE, PROFILE_IMAGE_ALT, RESUME_HREF } from '@/constants';
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div>
            ...
            >
              ...
              <span className="inline-block h-[1.15em] w-0.5 bg-brand-2 animate-caret" data-caret />
            </p>
            ...
            >
              {/* Deliberately an <a>, not a <Link>: this is an in-page jump on the
                  one route this component renders on, and public/js/main.js
                  preventDefaults it to run the eased scroll and hold the rail
                  entry. A <Link> would add the router's own click handler on
                  top of that and navigate as well. */}
          ...
          >
            <div className="pointer-events-none absolute inset-0" aria-hidden="true">
              <span
                className="absolute -inset-[4%] rounded-full border border-dashed border-line-strong
                opacity-60 animate-orbit-slow"
              />
              <span
                className="absolute inset-[7%] rounded-full border opacity-60 animate-orbit-mid
                border-[color-mix(in_srgb,var(--brand-2)_30%,transparent)]"
              />
              <span
                className="absolute inset-[16%] rounded-full border opacity-60
        ...
        {/* Same reason as the hero buttons above: an in-page jump main.js owns. */}
        {/* eslint-disable-next-line @next/next/no-html-link-for-pages */}
        <a
      ...
      >
        <div className="flex w-max animate-marquee [will-change:transform] group-hover:[animation-play-state:paused]">
          ...
          {[0, 1].map((group) => (
            <div
              className="flex items-center gap-11 pr-11 max-[640px]:gap-7 max-[640px]:pr-7"
              key={group}
            >
              {MARQUEE.map((item) => (
                <span
