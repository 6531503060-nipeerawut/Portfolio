...
/**
 * Who the site is, and the handful of values every route needs to say so.
 *
 * Nothing here describes what the site *says* — that is `constants/content.ts`
 * — and nothing here is a route: the two front ends and the table that maps
 * between them live in `constants/navigation.ts` and `constants/routes.ts`.
 */

/* ── Environment ──────────────────────────────────────────────────── */

/**
 * Which build this is. Written into the bundle at build time, never read
 * from the request, and checked against the file it came from by
 * `npm run check:env`.
 */
...
export const DESCRIPTION =
  'Peerawut Nipakornpan is a junior full-stack developer at Gendee.ai working across Angular, ' +
  'Next.js, Ionic, Deno edge functions and PostgreSQL — shipping course, billing, notification, ' +
  'B2B organization and news systems into production.';
...
/**
 * The résumés live in /public; keeping the paths in one place means the files
 * can be renamed without hunting through the markup. The `<a download>` tags
 * carry no filename of their own, so these basenames are also what land in
 * the visitor's downloads folder.
 */
export const RESUME_HREF = '/Resume_PeerawutNi.pdf';

/** The single-column copy written for applicant tracking systems. */
export const RESUME_ATS_HREF = '/Resume_PeerawutNi-ATS.pdf';
